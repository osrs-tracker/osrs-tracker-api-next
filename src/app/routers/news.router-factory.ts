...
export class NewsRouterFactory implements RouterFactory {
  ...
  create({ express, mongo }: App): void {
    ...
    express.use('/news', router);
  }

  private setupRoutes(router: Router, mongo: MongoClient): void {
    this.newsProxy(router, mongo);
  }

  /**
   * PROXY TO OLD API FOR BACKCOMP SUPPORT.
   */
  private newsProxy(router: Router, mongo: MongoClient): void {
